...
//! # `llm-d` Only
//!
//! This crate is an AI-owned compatibility layer exclusively for
...
//! - Other use cases require a maintainer discussion/request on `GitHub`.
//!
...
//! # Overview
//!
//! Provides an [`HttpFilter`] that sends request and response data
//! to an external gRPC server via the Envoy [`ext_proc`] protocol.
//! The configuration surface mirrors Envoy's [`ExternalProcessor`]
//! proto.
//!
//! # Registration
//!
//! This filter is not included in [`FilterRegistry::with_builtins`].
//! Register it explicitly (feature-gated in Praxis AI):
//!
//! ```ignore
...
//!         praxis_ai_llmd_ext_proc::ExtProcFilter::from_config,
//!     ),
//! ).unwrap();
//! ```
//!
...
mod mutations;
/// Protobuf and gRPC types for the Envoy `ext_proc` protocol.
///
/// **`llm-d` only:** These types are exposed for `llm-d` integration
/// tests that implement mock `ExternalProcessor` servers. No API
/// stability is guaranteed for non-`llm-d` use cases.
pub mod proto;
